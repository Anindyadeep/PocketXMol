...
def parse_qvina_outputs(docked_sdf_path, ref_mol):
    ...
    for i, mol in enumerate(suppl):
        ...
        break  # only use the first vina result
...
class QVinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_mol_data(cls, ligand_rdmol, ligand_filename, protein_root='./data/crossdocked', **kwargs):
    ...
    def __init__(self, pdb_block, ligand_rdmol, conda_env='adt', tmp_dir='./tmp', use_uff=True, center=None):
        ...
        self.noH_rdmol = Chem.RemoveHs(deepcopy(ligand_rdmol))  # calc rmsd with init mol (no uff)
