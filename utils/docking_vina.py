...
def pdbqt_to_sdf(pdbqt_block, ref_ligand_path):
    #  pdbqt to pdb using pybel
    pdbqt_mol = pybel.readstring('pdbqt', pdbqt_block)
    pdb_block = pdbqt_mol.write('pdb')
    # pdb to sdf using rdkit
    rdkit_mol = Chem.MolFromPDBBlock(pdb_block)
    if ref_ligand_path.endswith('.pdb'):
        ref_ligand = Chem.MolFromPDBFile(ref_ligand_path)
    else:
        ref_ligand = Chem.MolFromMolFile(ref_ligand_path)
    if rdkit_mol is not None and ref_ligand is not None:
        sdf_mol = AllChem.AssignBondOrdersFromTemplate(ref_ligand, rdkit_mol)
        return sdf_mol
    else:
        return None
    ...
    def dock(self, score_func='vina', seed=0, mode='dock', exhaustiveness=8, save_pose=False, **kwargs):  # seed=0 mean random seed
        ...
        if not save_pose: 
            ...
            if mode == 'score_only': 
                ...
                raise ValueError
            if self.ref_ligand_path and pose is not None:
                pose_sdf = pdbqt_to_sdf(pose, self.ref_ligand_path)
                if pose_sdf is not None:
                    return score, pose_sdf
                else:
                    print('pdbqt_to_sdf failed')
                    return score, pose
            return score, pose
...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_mol(cls, ligand_rdmol, protein_fn, protein_root='./data/crossdocked', **kwargs):
        # load original pdb
        protein_path = os.path.join(protein_root, protein_fn)
        return cls(protein_path, ligand_rdmol, **kwargs)
    ...
    def run(self, mode='dock', exhaustiveness=8, **kwargs):
        ...
        save_pose = kwargs.pop('save_pose', True)
        results = dock.dock(score_func='vina', mode=mode, exhaustiveness=exhaustiveness, save_pose=save_pose, **kwargs)
        if save_pose:
